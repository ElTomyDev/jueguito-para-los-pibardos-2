...
class ActorCritic(nn.Module):
    ...
    def get_action_and_logprob(
        self, state, actor_hidden, _critic_hidden=None, deterministic=False
    ) -> tuple:
        ...
        if deterministic:
            ...
            log_prob = (
                move_dist.log_prob(move_xy).sum(-1)
    ...
    def evaluate(self, states, actions, actor_hidden, _critic_hidden=None) -> tuple:
        ...
        move_xy = torch.stack(
            [actions["move_x"].reshape(B * T), actions["move_y"].reshape(B * T)], dim=-1
        )
...
class RolloutBuffer:
    ...
    def get_training_batches(
        ...
        if extra_states and len(extra_states) >= T:
            ...
            win_starts = win_pool[
                : win_per_batch * (len(starts) // chunks_per_batch + 1)
            ]
        ...
        for i in range(0, len(starts), chunks_per_batch):
            ...
            if extra_states and win_starts:
                win_chunk_idx = (i // chunks_per_batch) % max(
                    1,
                    len(win_starts) // max(1, int(chunks_per_batch * WIN_REPLAY_RATIO)),
                )
                for wc in range(int(chunks_per_batch * WIN_REPLAY_RATIO)):
                    ws_idx = (
                        win_chunk_idx * int(chunks_per_batch * WIN_REPLAY_RATIO) + wc
                    ) % max(1, len(win_starts))
            ...
            actions_t = {
                ...
                "shot_angle": torch.FloatTensor(np.array(a_dict["shot_angle"])).to(
                    device
                ),
...
class PPOAgent:
    def __init__(self, config) -> None:
        ...
        self.optimizer = optim.Adam(
            self.network.parameters(), lr=config["lr"], eps=config.get("adam_eps", 1e-5)
        )
    ...
    def get_action(self, state: list, deterministic: bool = False) -> tuple:
        ...
        with torch.no_grad():
            action_dict, log_prob, value, new_ah, _ = (
                self.network.get_action_and_logprob(
                    s_t, self.actor_h, None, deterministic
                )
            )
    ...
    def store_transition(
        self, state, action, reward, done, log_prob, value, ah, _ch=None
    ) -> None:
    ...
    def update(
        self, last_value: float = 0.0, boss_won: bool = False, episode_data: dict = None
    ) -> None:
        ...
        if boss_won and episode_data:
            ...
            print(
                f"  → Victoria guardada en win_buffer ({len(self.win_buffer.episodes)}/{WIN_BUFFER_MAX})"
            )
        ...
        for _ in range(self.config["epochs"]):
            for (
                ...
                ah_b,
                _,
            ) in self.buffer.get_training_batches(
                advantages,
                returns,
                ...
                loss = (
                    actor_loss + self.config["value_coef"] * value_loss + entropy_loss
                )
    ...
    def end_episode(
        ...
        if timed_out and last_state is not None:
            ...
            with torch.no_grad():
                mx, my, a_mean, d_logits, new_ah = self.network.forward_actor(
                    s_seq, self.actor_h
                )
        ...
        print(
            f"Ep {self.episode_count:4d} | avg20: {avg:8.1f} | "
    ...
    def _update_entropy_coef(self) -> None:
        ...
        self.entropy_coef = (
            self.config["entropy_coef_start"] * (1 - t)
            + self.config["entropy_coef_end"] * t
        )
